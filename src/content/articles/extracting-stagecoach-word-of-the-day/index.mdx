...
```java {2} title=com.stagecoach.stagecoachbus.views.home.ticketview.activeticketview.AbstractQrActiveTicketView.java
...
```java title=com.stagecoach.core.model.database.word.Word.java
public class Word {
...
```java title=com.stagecoach.stagecoachbus.logic.DatabaseProvider.java
...
```java title=com.stagecoach.core.logic.WordOfTheDayFile.java
...
```java title=com.stagecoach.core.cache.SecureUserInfoManager.java
...
```c++ "(*(*param_1)->GetStringUTFChars)" "JNIEnv *param_1" title=libticket-ref-code.so
...
```java title=com.stagecoach.core.utils.AES256Cipher.java
...
```python title=decryption.python
import base64
